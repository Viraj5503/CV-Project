...
- 2026-07-05: **Task 5 COMPLETE — Phase 1 SHIPPED.** Repo renamed by Viraj to `PCB-Defect-Detection` (public; local remote URL updated). Added `README.md` (badges, hero gallery, metrics tables, Mermaid architecture, Docker-one-liner quickstart with release-weights download, API example, dataset + roadmap), `LICENSE` (MIT), `docs/resume_bullets.md` (4 quantified bullets + short variants + interview talking points). GitHub **Release v1.0.0** created via API (git-credential token; `gh` not installed) with `best.pt` (19.3 MB) as asset since weights are gitignored. Verified: 20/20 tests pass locally **and from a fresh GitHub clone**; weights downloaded via the README's release URL sha256-match local `best.pt`; raw README on GitHub byte-identical to local. Live at https://github.com/Viraj5503/PCB-Defect-Detection. Next: Phase 2 (RAG add-on) after exams.
