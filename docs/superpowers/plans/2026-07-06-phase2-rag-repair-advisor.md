...
**Amendment (2026-07-10, approved):** full RAGAS over the 9-config grid ≈ 800–1,000 Gemini calls — exceeds free-tier daily quota. Two-stage eval instead: **Stage 1** tunes the grid with local, LLM-free retrieval metrics (hit-rate@k + MRR against per-case `relevant_doc_ids` annotations); **Stage 2** runs full RAGAS (Gemini judge, rate-limited) only on the winning config + the current 800×4 default as baseline. Same deliverables.

**Steps:**

- [x] Add `ragas` (+ LangChain Gemini wrapper it needs for the judge) to `requirements.txt`; pip install into `.venv`; verify imports
- [x] Author `eval/rag_testset.jsonl` — ~30 hand-written cases covering all 6 classes + general practice: `{question, defect_class, ground_truth, relevant_doc_ids}`; questions span acceptability / rework / overview; ground truths written from the actual KB docs
- [x] TDD `tests/test_rag_evaluate.py` (mock/stub, no network): testset load + validation (bad class / missing field rejected), hit-rate@k + MRR math on canned retrieval results, grid runner assembles one row per config (stub embeddings, tmp KB), RAGAS sample mapping from `recommend()` output (provider mocked), report writer renders metric table + chosen config
- [x] Implement `src/pcb_vision/rag/evaluate.py` (`load_testset`, `retrieval_metrics`, `run_grid`, `ragas_eval`, `write_report`, CLI `python -m pcb_vision.rag.evaluate --stage grid|ragas|all`); tests green
- [x] Stage 1 live: build temp index per chunk_size (real MiniLM), score all 9 configs on hit-rate@k + MRR; pick winner
- [x] Stage 2 live: RAGAS faithfulness + answer relevancy + context precision with Gemini judge (throttled) on winner + 800×4 baseline
- [x] Settle defaults from the evidence (RAGAS says stage-1 winner ≈ baseline: 800×4 kept, no churn); write `reports/rag_evaluation.md`
- [x] Full suite (`pytest`) green; update Status log; commit (plain style)
...
- 2026-07-11: **Task 3 COMPLETE.** ragas 0.4.3 (langchain stack pinned <1 — ragas imports the 0.3 line). `eval/rag_testset.jsonl`: 30 hand-written cases, all 6 classes + general, annotated `relevant_doc_ids`. `rag/evaluate.py` built TDD (13 new tests; 57/57 suite). **Stage 1** (local hit-rate@k/MRR, 9 configs): 800/1200 saturate hit-rate 1.00; MRR 0.98 (1200) vs 0.96 (800) vs 0.90 (500) — grid pick 1200×3. **Stage 2** (RAGAS, 30 cases/config): winner 1200×3 = 0.97 faith / 0.81 relevancy / 0.79 precision; baseline 800×4 = **0.98 faith** / 0.83 / 0.76 — within noise, shipped default kept (no churn); **headline faithfulness 0.98**. Hard-won quota intel (live-probed): free tier post-Dec-2025 = 20 req/day on flagship flash (2.5, 3.5), 0 on 2.0-line, 2.5-flash-lite retired 404; only the lite line is usable (15 RPM, ≥500/day observed) → `GeminiProvider.DEFAULT_MODEL` + RAGAS judge switched to rolling `gemini-flash-lite-latest` (Viraj-approved; alias over pin since Google retires/requotas pinned models silently). Eval infra hardened en route: `ThrottledProvider` (RPM pacing + 429 retry), per-config report checkpointing, persisted generation samples (`reports/rag_samples_*.jsonl`, gitignored) with resume, `ResponseRelevancy(strictness=1)` (lite models reject multi-candidate). Next: Task 4 (`/recommend` endpoint + Streamlit Repair Advisor tab + Docker).
- 2026-07-08: **Task 2 COMPLETE.** google-genai 2.10.0 added. `rag/providers.py` + `rag/generate.py` built TDD (15 new tests, mock provider records prompts). `provider_from_env()`: explicit `PCB_RAG_PROVIDER` wins; unset auto-detects anthropic-then-gemini from keys; selected-but-keyless degrades to None (extractive) instead of raising; unknown name is a hard ValueError. `recommend()` enforces the prompt contract (numbered context, [1]..[k] citations, "not covered by the knowledge base"), skips the LLM entirely on empty retrieval, and maps citations[n-1] ↔ [n]. Spec deviation (improvement): default question uses human-readable `CLASS_LABELS` — bare "short" retrieved only general chunks (MiniLM reads it as an adjective); "short circuit" pulls short_overview/short_acceptability to the top. Live verification with real Gemini free-tier key (gemini-2.5-flash) against the real 104-chunk index: grounded, correctly-cited answers for open_circuit and short; model honestly noted short_rework wasn't in top-4 (existing Task 3 ranking target). Also live-confirmed the Anthropic-first auto-detect path (hit the real no-credits 400 — `PCB_RAG_PROVIDER=gemini` in `.env` is the working config). Full suite **44/44**. `.env.example` (+`PCB_RAG_PROVIDER`, `GEMINI_API_KEY`, `PCB_RAG_MODEL`). Next: Task 3 (RAGAS evaluation with Gemini as judge; grid over chunk_size × top_k; `eval/rag_testset.jsonl` ~30 hand-written cases).
